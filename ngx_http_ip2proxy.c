	...
	ngx_http_ip2proxy_create_ctx(ngx_http_request_t *r) {
		...
		if (xfwd->nelts > 0 && cfg->reverse_proxy) {
            ...
            if (addr.len > (NGX_INET6_ADDRSTRLEN + 1)) {
               fprintf(stderr, "new1============================================addr.len: %zu vs %zu, addr.data: %p\n", addr.len, (NGX_INET6_ADDRSTRLEN + 1), addr.data);
               fprintf(stderr, "new2============================================addr.len: %zu vs %zu, addr.data: %p\n", addr.len, (NGX_INET6_ADDRSTRLEN + 1), addr.data);
               fprintf(stderr, "new3============================================addr.len: %zu vs %zu, addr.data: %p\n", addr.len, (NGX_INET6_ADDRSTRLEN + 1), addr.data);
               u_char test_ip_addr[124];
               (void) ngx_copy((void *)test_ip_addr, addr.data, addr.len);
               test_ip_addr[addr.len] = '\0';
               fprintf(stderr, "new============================================addr.data: %s\n", (char*)test_ip_addr);
            }
